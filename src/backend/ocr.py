...
def main():
    ...
    print("=== PDF EXTRACTOR v2.0 (Умный помощник) ===")
    print("1 - Обработать один PDF файл")
    print("2 - Обработать папку с PDF")
    print("3 - Показать статистику")
    
    choice = input("Выберите режим: ").strip()
    
    if choice == "1":
        pdf_path = input("Путь к PDF: ").strip()
        if not Path(pdf_path).exists():
            print("Файл не найден")
            return
        
        interactive = input("Использовать интерактивный режим? (да/нет): ").lower() == "да"
        result = extractor.extract(pdf_path, interactive=interactive)
        
        print(f"\nРезультат: {'УСПЕШНО' if result['success'] else 'НЕ УДАЛОСЬ'}")
        print(f"Метод: {result['method']}")
        print(f"Текст: {result['text'][:200] if result['text'] else 'Нет текста'}...")
        ...
        interactive = input("Использовать интерактивный режим? (да/нет): ").lower() == "да"
        results = extractor.extract_batch(folder_path, interactive=interactive)
        
        success = sum(1 for r in results if r['success'])
        print(f"\nОбработано: {len(results)} PDF файлов")
        print(f"Успешно: {success}")
        print(f"Неудачно: {len(results) - success}")
