...
[Скачать релиз](https://github.com/9podkir10-cmd/PDF-Toolkit/releases) · [Установка из исходного кода](#Установка-из-исходного-кода)
...
При первом запуске программа создаст файл `settings.json` и папку `ocr_storage` рядом с исполняемым файлом (или рядом с `main.py`).
Содержимое:

- `ocr_path` — путь к исполняемому файлу Tesseract (обязательно для OCR).
- `language` — язык распознавания: `rus`, `eng`, `ruseng`.

- `images/` — все вырезанные области в формате PNG (имя файла — случайный UUID).
- `metadata.json` — JSON-файл с метаданными:

Все данные хранятся локально, можно использовать для дообучения OCR
...
- **Windows** (основная платформа), хз работает ли на Linux/macOS (не тестировалось).
...
- Доработка механики выделения областей на изображениях.
- Добавление шаблонов областей распознавания и шаблонов переименования файлов.
- Обработка дубликатов и единая команда split.
