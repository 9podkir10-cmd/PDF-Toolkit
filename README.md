...
- добавлена функция `structure` для построения дерева структуры папок - анализирует manifest.json, строит дерево папок и показывает, сколько файлов попадёт в каждую папку при структуризации.
- Единая команда split и единый pipeline для индексации `index` - (index, ocr, structure)
- Добавлена вкладка `Scan` для работы со сканерами. Настройки сканирования + предпросмотр результата. Потоковый ввод.
- Добавлен dashboard
...
- Добавление шаблонов областей распознавания.
